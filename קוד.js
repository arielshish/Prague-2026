  ...
  if (!placesSheet) {
    ...
    var P = [
      ...
      ['d2_dancing',2,'attraction','💃 בית הריקוד – Dancing House (Tančící dům)',
        ...
        '','Medium','09:00-20:00 כל יום (תצפית גג)','חינם מבחוץ | 150 CZK לגג + בר','15-20 דקות',
      ...
      ['d4_aquapalace',4,'attraction','🌊 Aquapalace Prague – פארק מים ענקי',
        ...
        'https://www.aquapalace.cz/en/','High','09:00-22:00 (Water World) | כניסה אחרונה 20:00','650 CZK מבוגר / 530 CZK ילד (3-12)','כל היום!',
      ...
      ['d5_zoo',5,'attraction','🐘 גן חיות פראג – Prague Zoo',
        ...
        'https://www.zoopraha.cz/en','High','09:00-19:00 (קופות/כניסה) | שהייה עד 21:00','300 CZK מבוגר / 200 CZK ילד','4-5 שעות',
      ...
      ['d6_fashion',6,'shopping','🛍️ Fashion Arena Outlet – מכרה הנחות',
        ...
        '','Medium','10:00-20:00','חינם (הקניות לא)','2 שעות',
...
function saveDaysCustom(daysData) {
  ...
  try {
    ...
    var data = sheet.getDataRange().getValues();
    var found = false;
    var json = JSON.stringify(daysData);
    for (var i = 1; i < data.length; i++) {
      if (data[i][0] === 'days_custom') {
        sheet.getRange(i + 1, 2).setValue(json);
        found = true;
        break;
      }
    ...
    if (!found) {
      sheet.getRange(sheet.getLastRow() + 1, 1, 1, 2).setValues([['days_custom', json]]);
    }
    SpreadsheetApp.flush();
    return { ok: true };
  } catch(e) {
