...
**⚠️ באג ידוע שלא תוקן — נעצים כפולים במפה**: 15 מקומות מופיעים פעמיים ב-`ALL_PLACES` (פעם `type:'restaurant'`/`'shop'` ופעם `type:'community'` עם `cat:'🍽️ מסעדות'`), ושניהם עם אותו `name` → `PLACE_COORDS[name]` מחזיר קואורדינטות לשניהם ו-`addMapMarkers` מצייר **שני נעצים זהים זה על זה**. רשימה: Palladium, Westfield Chodov, Nový Smíchov, Albert Chodov, המזללה, Vapiano, Mango, Yammi Sushi, Výtopna, U Fleků, Las Adelitas, Manifesto Market, Café Imperial, Rabit, Černá Madona. **קיים מלפני 08/2026, לא נוצר ע"י התוספות של הרובע.** תיקון אפשרי: de-dupe לפי `name` ב-`addMapMarkers`/`renderNearbyList`.
