...
| `renderLocationTab()` | ~4460 | מאתחל מפה + רשימת קרובים |
| `initLeafletMap()` | ~4465 | retry loop עד Leaflet נטען, `invalidateSize()` |
| `addMapMarkers()` | ~4548 | pins לכל ALL_PLACES עם קואורדינטות |
| `showMapNavDialog(p,coords)` | ~4572 | bottom sheet ניווט (Google/Waze/מקומי) |
| `locateMe()` | ~4630 | Geolocation → pin "אתם כאן" |
| `renderNearbyList()` | ~4670 | רשימה ממוינת לפי מרחק |
| `haversine(...)` | ~4449 | מרחק בק"מ |
| `restaurantImgHtml(r)` | ~4350 | header תמונה עם fallback |

## פיצ'רים קיימים (עדכון 2026-07-18 — גרסה 9)

### Light Theme v7 (2026-07-18)
- עיצוב בהיר שולט: body `#F7F3EE`, כרטיסים/מודאלים/nav `#ffffff`
- טיסות: כרטיסים לבנים עם border צבעוני (כתום/כחול)
- day pills לא-פעילות: `#ffffff` + border
- header נשאר כהה (Prague skyline — accent עיצובי)
- גבולות לבנים `rgba(255,255,255,0.14/0.15)` → `rgba(0,0,0,0.10)` בכל הקובץ
...
- **CSS token layer** (~line 19): `:root` with full design tokens. Key: `--c-primary:#F4634A`, `--c-card:#fff`, `--r-card:22px`, `--s-card`, `--t-fast/base/slow`, `--ease-spring`
- **DS classes**: `.ds-badge-{red,amber,green,blue,gray}`, `.ds-chip`, `.ds-fab`, `.ds-stat`, `.ds-section-hdr`, `.skeleton`, `.btn-primary` (gradient + shadow), `.btn-ghost`
- **Bottom nav** (~line 1761): `<button class="nav-item">` with `.nav-icon`/`.nav-label`; `openTab()` toggles `.active` class
- **ימים** stop cards: DS tokens, numbered circle markers, time chip, `.ds-badge-amber` for Google, `.btn-primary` "הוסף תחנה"
- **תזכורות**: priority right-border (red/amber/blue), `.ds-badge-*` urgency chips
- **הוצאות**: stray "ביטול" removed; `.ds-stat` for summary cards
- **ציוד**: "איפוס" moved from header to progress row
- **טיסות**: check-in + airport-arrival info chips above cards
...
### טאב מיקום + מפת Leaflet (2026-07-18)
- **Leaflet.js** v1.9.4 מ-CDN (defer בhead) + OpenStreetMap tiles
- `PLACE_COORDS` (~שורה 4377) — קואורדינטות [lat,lng] לכל 105 המקומות ב-ALL_PLACES
  - כולל שתי גרסאות שם לכל spelling variant (גשר קארל/קרל, לטנה, ויישהראד וכו')
- `PLACE_IMGS` — Wikimedia Commons URLs עם onerror fallback
- `TYPE_GRAD` — צבעי gradient fallback לפי type
- `_leafletMap` — singleton, `initLeafletMap()` עם retry loop (עד 50 × 200ms)
- `addMapMarkers()` — מוסיף pins לכל ALL_PLACES עם קואורדינטות; צבעים לפי type + כתום אם בלוז
- **דיאלוג ניווט עשיר** (`showMapNavDialog(p, coords)`) — scrollable bottom sheet בלחיצה על כל pin:
  - header: icon + שם + תג ✅ "כבר בלוז · שעה" אם ב-DAYS
  - **מידע לפי type** (חלק עליון גלילה):
    - `community`: desc, chips (duration/google/who), הוראות הגעה, booking, tips[]
    - `restaurant`: badge, sub, desc, chips (price level / CZK / google⭐)
    - `dessert`: sub, desc, chips (tag / rating / price / where)
    - `shop`: chips (stars / hours / metro / duration), brands, tip
    - `photo`: sub, desc, chips (rating / best / crowds / fee_txt), tip
  - כפתורי ניווט נעוצים בתחתית: 🗺️ Google Maps | 🚗 Waze | 📍 מפה מקומית | סגור
- `locateMe()` — Geolocation API, מוסיף pin סגול "אתם כאן"
- `renderNearbyList()` — מיון לפי Haversine + פילטר קטגוריה
- `haversine(lat1,lng1,lat2,lng2)` — מרחק בק"מ
- pins צבעוניים: 🟠 כתום=בלוז | 🔵 כחול=מסעדות | 🟢 ירוק=קהילה | 🟣 סגול=צילום | 🟡 צהוב=קניות

### תמונות על כרטיסים (2026-07-18)
- `restaurantImgHtml(r)` — header תמונה 150px: gradient+emoji (always) + Wikimedia img (overlay, onerror remove)
- תמונה 90px גם על stop cards בלוז (`renderDays()`) — emoji+שם בoverlay, מספר בפינה
- `PLACE_IMGS` — lookup לפי שם מקום → Wikimedia Commons URL
