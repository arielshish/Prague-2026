...
if "return rows.sort(function(a,b){" in newfunc:
    newfunc = newfunc.replace("return rows.sort(function(a,b){", "return _tripSummaryDedupeRows(rows).sort(function(a,b){", 1)
elif "return rows;" in newfunc:
    newfunc = newfunc.replace("return rows;", "return _tripSummaryDedupeRows(rows);", 1)
else:
