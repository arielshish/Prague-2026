...
**שני מגנים:**
- **`_hasLocalData(key)`** — resync מעלה למעלה **בלבד**. אם אין נתונים מקומיים אין מה לתרום, ודחיפה היא רק הרס. מזהה גם `{}`/`[]` ולא רק מפתח חסר. חל על visited, ימים, הוצאות, ציוד ותזכורות.
